...
function calcularValorParte(parte) {
    let valor;
    valor = parte.cantidad * parte.precio_unitario;   
    return valor;  
...
function calcularTotalInventario(listado) {
    ...
    acum = acum + calcularValorParte(parte) 
    })
    return acum
...
function registrarEntrada(idParte, cantidad) {
    ...
    let ncantidad;
    ncantidad = parte.cantidad + cantidad

    if (ncantidad > parte.capacidad_maxima) {
        return false;
    }

    parte.cantidad = ncantidad
        return true;
...
function registrarSalida(idParte, cantidad) {
    ...
    let rcantidad;
    rcantidad = parte.cantidad - cantidad

    if (rcantidad < 0) {
        return false;
    }
    
    parte.cantidad = rcantidad
        return true;
...
function calcularPorcentajeOcupacion(parte) {
    let porcentaje;
    porcentaje = (parte.cantidad / parte.capacidad_maxima) * 100
